...
This example samples an analog CH01 gas sensor, restores the sensor voltage before the board's 2:1 resistor divider, estimates concentration, and reports a 30-second concentration trend.
...
The sketch enables board-controlled sensor power when available, configures a 12-bit ADC, and averages 16 samples for each reading. ESP32-based boards use `analogReadMilliVolts()`; RC52 uses the 3.0 V internal reference and converts the averaged raw ADC value to millivolts.
